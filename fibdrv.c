...
static uint64_t fib_fast_doubling_clz(uint64_t k)
{
    uint64_t a = 0, b = 1;

    for (uint64_t i = k ? 1llu << (63 - __builtin_clzll(k)) : 0; i; i >>= 1) {
        uint64_t t1 = a * (2 * b - a);
        uint64_t t2 = a * a + b * b;
        a = t1;
        b = t2;
                        ...
                        loff_t *offset)
{
    ...
    if (choice == 0)
        ...
        fib_compute = fib_fast_doubling_clz;
    else
        return -1;
